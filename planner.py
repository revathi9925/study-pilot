...
def calculate_priority(subject, today=None):
    ...
    # Parse the weightage
    try:
        weightage = float(weightage_str.replace("%", "").strip())  # FIXED: was .find("%", "")
    except:
        weightage = 10.0
...
def generate_weekly_plan(allocated_subjects, daily_hours=4, days_ahead=7):
    ...
    for subject in allocated_subjects:
        subjects_summary += f"""
                Subject : {subject['subject']} - {subject.get('unit', '')}
    ...
    prompt = f"""
            ...
            5. Return ONLY valid JSON - no explanation, no markdown, no trailing commas.
...
def clean_json_response(raw):
    # FIXED: strip markdown code blocks if model wraps response in them
...
def main():
    ...
    print(f"✅ Loaded {len(subjects)} subject(s) successfully!")
